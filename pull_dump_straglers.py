...
stragler = open("stranglers.txt", "r")
...
with open(stragler) as line:
	try:
		...
		Get Cookie
		r = requests.get('http://grcourt.org/CourtPayments/loadCase.do?caseSequence=' + str(line))
		headers = r.headers['set-cookie']
		print headers
		...
		#Request Page
		f = codecs.open(str(line) + ".html", "r",encoding='utf-8')
		...
		if data_ccsort.string == u'Civil Case View':
			...
			count += 1
			if count % 100 == 0:
				print "We are on record: " + str(count)
			else:
				continue
	except (AttributeError, IOError, UnicodeEncodeError):
#
			#time.sleep(2.5)
stragler.close()			